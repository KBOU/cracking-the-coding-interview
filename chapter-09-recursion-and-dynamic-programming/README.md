...
## 9-10. 箱を積み重ねて最も高くなる高さを求める。

条件

* 上に載せる箱は長さ、高さ、奥行きすべて下の箱より小さくないとダメ

### 方針

1. Heightの大きい順にソートする
2. Width, Depthでi番目とi-1番目の序列をチェックする。Wi-1 > Wi かつ Di-1 > Diなら足し上げる
3. 違った場合は、枝分かれする。
...
## 9-11. 0,1,&,|,^のoperator, operandの文字列が結果(True, False)となるような()の場合の数を求める

よくわからんですね。  
例えば、文字列"1^0|0|1"と 結果0が渡された場合、  
適宜優先順位をつけて演算結果を0にする優先順位付けは何個あるかという話です。
...
1. 与えられた文字列のOperatorを前から順に抽出
2. 抽出されたOperatorの左側の文字列と左側に求める結果を与えて再帰する。(右側も)
3. 左側と右側の場合の数を掛け合わせてその文字列の場合の数とする。
